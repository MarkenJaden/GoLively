    ...
    public override (string androidPermission, bool isRuntime)[] RequiredPermissions
    {
        get
        {
            ...
            if (sdk >= 31)
            {
                ...
                if (_scan)
                    result.Add((Android.Manifest.Permission.BluetoothScan, true));

                if (_advertise)
                    result.Add((Android.Manifest.Permission.BluetoothAdvertise, true));

                if (_connect)
                    result.Add((Android.Manifest.Permission.BluetoothConnect, true));

                if (_bluetoothLocation)
                    result.Add((Android.Manifest.Permission.AccessFineLocation, true));
            }
            else
            {
                // If your app targets Android 11 (API level 30) or lower, declare the following permissions in your app's manifest file:

                result.Add((Android.Manifest.Permission.Bluetooth, true));
                ...
                {
                    result.Add((Android.Manifest.Permission.BluetoothAdmin, true));
                    ...
                    if (sdk >= 29)
                    {
                        ...
                        result.Add((Android.Manifest.Permission.AccessBackgroundLocation, true));
                    }
                }
            }
